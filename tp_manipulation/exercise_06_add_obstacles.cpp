/**
 * Exercise 6: Add two obstacles in the planning scene and avoid them using
 * Franka Panda robot Based on:
...
void addBoxObstacle(moveit::planning_interface::PlanningSceneInterface
  ...
  // TODO: Create a CollisionObject message, fill it and add it to the planning
  // scene
...
bool planAndExecute(
  ...
  if (plan_solution) {
    visual_tools.publishTrajectoryLine(plan_solution.trajectory,
    ...
    if (success) {
      ...
      visual_tools.trigger();
      // RCLCPP_INFO(RCLCPP_GET_LOGGER(LOGNAME), "\u2713 %s execution
      // succeeded!",
      //             description.c_str());
    } else {

      // RCLCPP_WARN(RCLCPP_GET_LOGGER(LOGNAME), "\u2717 %s execution failed!",
      //             description.c_str());
    }
    return success;
  } else {
    // RCLCPP_ERROR(RCLCPP_GET_LOGGER(LOGNAME), "\u2717 %s planning failed!",
    //            description.c_str());
    return false;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  // Obstacle 1
  // TODO: Define box1 shape and pose, then add it to the planning scene with
  // addBoxObstacle function

  auto frame_id1 = "fr3_link0";

  shape_msgs::msg::SolidPrimitive box1;
  box1.type = box1.BOX;
  box1.dimensions = {0.15, 0.15, 0.3};

  geometry_msgs::msg::Pose box_pose1;
  box_pose1.position.x = 0.45;
  box_pose1.position.y = -0.25;
  box_pose1.position.z = 0.15;

  addBoxObstacle(planning_scene_interface, frame_id1, "box1", box1, box_pose1);

  // TODO : Define box2 shape and pose, then add it to the planning scene with
  // addBoxObstacle function Obstacle 2

  auto frame_id2 = "fr3_link0";

  shape_msgs::msg::SolidPrimitive box2;
  box2.type = box2.BOX;
  box2.dimensions = {0.15, 0.15, 0.3};

  geometry_msgs::msg::Pose box_pose2;
  box_pose2.position.x = 0.45;
  box_pose2.position.y = 0.25;
  box_pose2.position.z = 0.15;

  addBoxObstacle(planning_scene_interface, frame_id2, "box2", box2, box_pose2);

  RCLCPP_INFO(logger,
              "Two collision objects applied to move_group planning scene");
  ...
  auto const target_pose = [] {
    ...
    msg.pose.position.y = -0.45; // <---- This value was changed
    msg.pose.position.z = 0.05;
    return msg;
  }();

  planAndExecute(planning_components, visual_tools, target_pose,
                 joint_model_group_ptr, moveit_cpp_ptr,
